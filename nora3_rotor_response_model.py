...
# -*- coding: utf-8 -*-
"""
Created on Thu Oct 10 14:33:15 2024
...
#%%
# Path to 1) Sørlige Nordsjø II 2) Utsire Nord
#merged_wind_file = '/Users/hermanellingsrud/Documents/Skole/Masteroppgave/Koder/data/Sørvest_F/merged_wind_data_sørlig_nordsjø_2_10_years.csv' #Sørlige Nordsjø II
merged_wind_file = "/Users/hermanellingsrud/Documents/Skole/Masteroppgave/Koder/data/Vestavind_F/merged_wind_data_utsira_10_years.csv" #Utsira


# Read the CSV file
df_wind = pd.read_csv(merged_wind_file)
df_wind['time'] = pd.to_datetime(df_wind['time'])  # Convert 'time' column to datetime format

# Define specific timestamp with a wind profile
specific_time = '2023-08-16 06:00:00'  # 3 hour resolution so only works with 00:00:00, 03:00:00, 06:00:00, 09:00:00, 12:00:00, 15:00:00, 18:00:00, 21:00:00
specific_time = pd.to_datetime(specific_time)

# Filter the dataset for the specified timestamp
filtered_wind_df = df_wind[df_wind['time'] == specific_time]

# Heights and corresponding wind speeds
heights = np.array([10, 20, 50, 100, 150, 200, 250, 300])  # Heights up to 300 m

wind_speeds = np.array([
    filtered_wind_df['wind_speed_10m'].values[0],
    filtered_wind_df['wind_speed_20m'].values[0],
    filtered_wind_df['wind_speed_50m'].values[0],   # Use wind_speed_50m_x for SNII if needed
    filtered_wind_df['wind_speed_100m'].values[0],  # Use wind_speed_100m_x for SNII if needed
    filtered_wind_df['wind_speed_150m'].values[0],
    filtered_wind_df['wind_speed_200m'].values[0],
    filtered_wind_df['wind_speed_250m'].values[0],
    filtered_wind_df['wind_speed_300m'].values[0]
])

# Plot the wind profile
plt.figure(figsize=(8, 10))
plt.plot(wind_speeds, heights, marker='o', linestyle='-', color='black')
plt.xlabel("Wind Speed (m/s)", fontsize=18)
plt.ylabel("Height (m)", fontsize=18)
plt.title(f"Vertical Wind Profile at {specific_time}", fontsize=20)
...
#%% Blade properties such as sections, twist, chord length, etc.
...
# Create empty lists to store values for various blade properties
blade_span = []
blade_curve_ac = []
blade_sweep_ac = []
blade_curve_angle = []
...
with open(file_path_airfoil_properties, 'r') as file:
    # Skip the header/definition lines in the dataset
    for _ in range(6):
        file.readline()
...
#%% Rotor data such as power, rotational speed, pitch angle, etc.
...
# Function that uses Ct from rotor data to determine axial induction factor
def get_axial_induction_factor(wind_speed, wind_speeds_performance, ct_values):
    ...
    Args:
        wind_speed (float): The current wind speed at hub height.
...
# Manually adjust pitch angle values to fine-tune performance
...
a_prime = 0  # Assumed to be zero in this study
...
rot_speed_hub = np.where(
    ...
    0, 
    np.interp(wind_speed_hub, wind_speeds_preformance, rotor_speeds) # interpolating from the turbine data
)
...
#%% Air foils - retrieving cl and cd for the corresponding angle of attacks
...
# Initialize arrays for phi values, and other parameters for each section
...
for section in range(num_blade_sections_50):
    ...
    for i, blade_position in enumerate(blade_positions):
        ...
        # Interpolate the blade pitch using the hub wind speed and performance data
...
def print_turbine_report(
    ...
    print("Wind Turbine Performance Recap", specific_time)
    print("--------------------------------")
...
plt.title(f"Flapwise Bending Moment {specific_time}", fontsize=20)
plt.legend(
    fontsize=17,
...
plt.xlabel("Blade Position (degrees)", fontsize=18)
plt.ylabel("(MNm)", fontsize=18)
plt.title(f"Edgewise Bending Moment {specific_time}", fontsize=20)
